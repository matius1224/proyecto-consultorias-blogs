            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createDataModalLabel">
                        {{ $selected_id ? 'Editar articulo' : 'Crear articulo' }}
                    </h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group">
                                <label for="ruta_archivo">* Articulo</label>
                                <input type="file" wire:model.live="ruta_archivo" class="form-control"
                                    accept=".pdf">
                                @error('ruta_archivo')
                                    <p style="color: red">{{ $message }}</p>
                                @enderror
                                @if ($ruta_archivo && $crear_articulo && !is_string($ruta_archivo))
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <h3 class="text-center">
                        {{ $activar ? '¿Estas seguro de activar el articulo?' : '¿Estas seguro de inactivar el articulo?' }}
                    </h3>
                </div>
                <form wire:submit="{{ $activar ? 'activarArticulo' : 'inactivarArticulo' }}({{ $selected_id }})">
