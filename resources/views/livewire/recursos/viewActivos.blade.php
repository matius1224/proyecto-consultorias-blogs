        ...
        <div class="card-body">
            <div class="container-botones-tabla-activos">
                ...
                <div class="container-boton-sincronizar">
                    <button id="boton-sincronizar" wire:click="openModal"><i class="fa fa-plus"></i> Nuevo recurso</button>
                </div>
            ...
            <div class="float-end">{{ $recursos->links() }}</div>
            <div class="float-start">Mostrando {{ $recursos->count() }} registros de un total de {{ $total }}
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        Livewire.on('eventoCrear', () => {
            Swal.fire({
                title: 'Coyca',
                text: 'Recurso creado éxitosamente',
                icon: 'success',
                timer: 3000,
                showConfirmButton: false
            }).then(() => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        Livewire.on('eventoActualizar', () => {
            Swal.fire({
                title: 'Coyca',
                text: 'Recurso actualizado éxitosamente',
                icon: 'success',
                timer: 3000,
                showConfirmButton: false
            }).then(() => {
